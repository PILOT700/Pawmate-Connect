...
const MAINZ = { cityLat: 49.99, cityLng: 8.24 }; // ~20 miles from Frankfurt
const LISBON = { cityLat: 38.72, cityLng: -9.14 }; // ~1200 miles away
// Frankfurt's latitude exactly, far to the east — about 500 miles out. Frankfurt
...
describeWithDb("the discover feed", () => {
  ...
  it("keeps the nearby city and drops the far one", async () => {
    ...
    expect(names((await feedFor(me.id, "?maxDistanceMiles=50")).body)).toEqual(["Nearby"]);
  });
  ...
  it("keeps someone whose city could not be placed", async () => {
    ...
    expect(names((await feedFor(me.id, "?maxDistanceMiles=50")).body)).toEqual(["Unplaced"]);
  });
  ...
  it("shows the whole feed to a viewer with no city of their own", async () => {
    ...
    expect(names((await feedFor(me.id, "?maxDistanceMiles=5")).body)).toEqual([
      "FarAway",
      "Nearby",
    ]);
  });
